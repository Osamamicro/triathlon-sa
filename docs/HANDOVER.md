...
| Production code (Weeks 1–4) | `src/Triathlon.Web` + `tests/Triathlon.Tests` (solution `Triathlon.slnx`). 215 tests green on PostgreSQL and SQL Server (CI runs both). Every public page renders from the database in `en` + `ar` via `Bi(en, ar)` (single-culture markup): home (CMS `home` page + KPIs + events + news), events list/detail/guest entry, season timeline + Kingdom map, join, athlete registration (Pending athlete + email, Turnstile when configured), training + guides, rules, governance + documents library (facets, counting downloads), statistics, news, contact, branded 404/500. Minimal APIs: `/api/timeline`, `/api/calendar.ics`, `POST /api/events/{slug}/register`, `POST /api/register`, `/documents|rules|training/{id}/download`, `/sitemap.xml`, `/robots.txt`. Security headers + CSP `script-src 'self'`, per-scope DB provider resolution, migration bundles in CI, bilingual dashboard sign-in (cookie culture). Redesign around the Federation brand (`docs/design/2026-09-07-redesign.md`). Lighthouse mobile perf 96–99 / a11y 100 on the gate pages (`docs/qa/lighthouse/2026-09-07-summary.md`). **Week 4 (content dashboard, code complete):** every content type has a list + edit screen in the Blazor dashboard (`Areas/Dashboard/Pages/{Events,Cities,Documents,Rules,Guides,Media,Pages,News,Governance,Navigation,Statistics,Settings}`, Overview tiles) on one page pattern (`EntityTable`, `EditShell` with field-anchored refusals and a list of unclaimed messages, `DashboardScope` per-operation DI scope carrying the acting user, `ReorderList`, `ItemsEditor` blocks editor); max-length validation on every bounded column and a `DbUpdateException` guard so no save can tear down a circuit; featured-event card CSS. 340 tests green on PostgreSQL at `3d79f13`; SQL Server last confirmed 336/336 at `f51bc93` (the final fix wave has not been run on SQL Server yet); both migration sets in sync (no schema change in Week 4). |
...
| Branch | `feat/week-4-dashboard` (from `main` after PR #5), head `11cc004`, pushed, **no PR yet — paused 2026-09-09 one step before the chain** (see §9). Week 5 step plans are committed at `docs/superpowers/plans/2026-09-08-task-4.{1,2}-*.md`. |
...
- 2026-09-09 — **Stage 3 (Week 4 — Content dashboard) code complete, PAUSED before the chain** at the user's request. Branch `feat/week-4-dashboard`, head `11cc004`, pushed, no PR yet. Plan 3.1 whole-branch-reviewed at `178ddfc`; plan 3.2 Tasks A–E each task-reviewed with one fix round; QA `docs/qa/2026-09-08-week4-dashboard.md` (P0+P1 fixed in `f51bc93`, F7–F14 deferred); whole-branch final review (opus) found no Critical and four Important items, fixed in `3d79f13` (anchored refusals + EditShell unclaimed-message list, media upload through `DashboardScope` with the `DbUpdateException` guard, alt-text length guard, required page/news titles). 340 tests on PostgreSQL. **Still open to close the stage (resume here):** (1) `STF_TEST_DB=SqlServer dotnet test` on `3d79f13` (last SQL Server run 336/336 at `f51bc93`); (2) a scoped re-review of the `f51bc93..3d79f13` fix wave; (3) the §7 chain — rewrite this line as "done", update §1's branch row, refresh memory `triathlon-session-state`, open the PR to `main`, spawn "Week 5: CRM + admin" (drafts for the PR body, memory text and spawn prompt exist in the session scratchpad `chain/` folder; regenerate from this section if lost). Decisions this stage: plan 3.1 "Decisions" 1–19, plan 3.2 "Decisions" 1–8, plus the 3.2 ledger rulings (`ActingUser` carried into the child scope; cascaded `FieldErrors` + `Name` on every kit input; Cities Key via `SlugField`; Media screen through `DashboardScope`; Pages slug rule = `SlugField Reserved` + companion hint; Statistics park-then-move; `ItemsEditor` mutable rows over the immutable `BlockItem`; `overflow-x` allowed on scroll containers; `Validation_MaxLength` + `ErrorUnexpected` guard; sonnet for polish/QA agents in efficient mode). Week 5 carry-overs are enumerated in the carry-over table at the top of `docs/superpowers/plans/2026-09-08-task-4.1-crm-domain-services.md` (§9 Stage-3 list, QA F7–F14, the polish pass's unfixed items, the review minors). Week 5 plans were adversarially reviewed once and rewritten; their code-symbol check is folded into the Week 5 SDD pre-flight scan. Client asks unchanged from Stage 2 plus a staging host for client review 2 and the licence validity rule. Process note: subagents must run `dotnet test` in the background and poll (blocking runs over 600 s trip the watchdog); keep at most two subagents in flight. Not done: staging deployment (no host yet).
